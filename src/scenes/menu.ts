...
export default class MenuScene extends Scene {
  ...
  preload() {
    ...
    this.load.image('background-layer1', '/assets/images/background-layer1.png');
    this.load.image('background-layer2', '/assets/images/background-layer2.png');
    this.load.image('background-layer3', '/assets/images/background-layer3.png');
  }
  ...
  createBackground() {
    ...
    this.bg1 = this.add.tileSprite(0, 0, this.width, this.height, 'background-layer1').setOrigin(0, 0);
    this.bg2 = this.add.tileSprite(0, 0, this.width, this.height, 'background-layer2').setOrigin(0, 0);
    this.bg3 = this.add.tileSprite(0, 0, this.width, this.height, 'background-layer3').setOrigin(0, 0);

    this.logo = this.add.image(this.width/2, this.height/2, 'logo');
  }

  createStartButton() {
    const buttonContainer = this.add.container(this.width/2, this.height * 4/5);
    ...
    const text = this.add.text(-110, -30, "Start!", {fontFamily: "EdMundMcMillen", fontSize: "4rem", color: "#000000"});
    buttonContainer.add(text)

    buttonContainer.setInteractive(new Geom.Rectangle(-120, -40, 240, 80), Geom.Rectangle.Contains)
      .on("pointerover", () => {
        graphics.clear();
        graphics.lineStyle(20, 0xbf8b20).strokeRoundedRect(-120, -40, 240, 80, 20);
